...
VALID_ACTIONS = {"IGNORE", "REPLY", "SUSPICIOUS", "ARCHIVE", "MOVE_TO_FOLDER"}
TOPIC_CATEGORIES = {
    "PERSONAL_DIRECT",
    "FINANCE",
    "SALES_OUTREACH",
...
def _normalize_single_triage_output(raw_output: Any) -> dict[str, Any]:
    if isinstance(raw_output, TriageDecision):
        ...
        return {
            ...
            "target_folder": str(decision.get("target_folder", "")).strip(),
    ...
    if isinstance(raw_output, dict):
        return {
            ...
            "target_folder": str(raw_output.get("target_folder", "")).strip(),
    ...
    if isinstance(raw_output, str):
        ...
        try:
            ...
            if isinstance(parsed, dict):
                return {
                    ...
                    "target_folder": str(parsed.get("target_folder", "")).strip(),
        ...
        upper = text.upper()
        for action in ("REPLY", "IGNORE", "SUSPICIOUS", "ARCHIVE", "MOVE_TO_FOLDER"):
            if action in upper:
                return {
                    "action": action,
                    "category": "",
...
def run_triage_and_print(max_results: int = 10) -> list[dict[str, Any]]:
    ...
    for email in emails:
        ...
        target_folder = str(normalized.get("target_folder", "")).strip()
    ...
    for item in triage_results:
        ...
        folder_info = f" | folder={target}" if target and action == "MOVE_TO_FOLDER" else ""
        summary = item.get("summary", "")
        summary_info = f" | summary: {summary[:50]}..." if summary else ""
        LOGGER.info(
            "id=%s | action=%s | category=%s | confidence=%.2f | signals=%s | reason=%s%s%s",
            item.get("id", ""),
            action,
            item.get("category", ""),
