...
def archive_email(message_id: str) -> dict:
    ...
    try:
        ...
        return {"status": "ok", "action": "archived", "message_id": message_id}
    except HttpError as exc:
        LOGGER.error("Failed to archive message %s: %s", message_id, exc)
        return {"status": "error", "action": "archived", "message_id": message_id, "error": str(exc)}
...
def mark_email_read(message_id: str) -> dict:
    ...
    try:
        ...
        return {"status": "error", "action": "mark_read", "message_id": message_id, "error": str(exc)}


def move_to_label(message_id: str, label_name: str) -> dict:
    """Move email to a specific label (add label, remove from INBOX)."""
    service = get_gmail_service()
    label_id = get_or_create_label_id(label_name)
    try:
        result = service.users().messages().modify(
        ...
        LOGGER.info("Moved message %s to label '%s'", message_id, label_name)
        return {"status": "ok", "action": "moved", "message_id": message_id, "label": label_name}
    except HttpError as exc:
        LOGGER.error("Failed to move message %s to label '%s': %s", message_id, label_name, exc)
        return {"status": "error", "action": "moved", "message_id": message_id, "label": label_name, "error": str(exc)}
